...
class IWP_MMB_Core extends IWP_MMB_Helper
{
    ...
    function network_admin_notice()
    {
		...
		<p>The WP Supervisor client plugin has to be activated on individual sites. Kindly deactivate the plugin from the network admin dashboard and activate them from the individual dashboards.</p></div>';
    }
    ...
    function admin_notice()
    {
       /* IWP */
		...
		if(!empty($iwp_client_activate_key)){
			...
			echo '<tr><td align="center">Please deactivate and then activate WP Supervisor Client plugin.</td></tr>';
		}
    ...
    function update_client_plugin($params)
    {
        ...
        if ($download_url) {
            ...
            if (is_wp_error($result) || !$result) {
                return array(
                    'error' => 'WP Supervisor Client plugin could not be updated.'
                );
            } else {
                return array(
                    'success' => 'WP Supervisor Client plugin successfully updated.'
                );
            }
        }
        return array(
    ...
    function client_replace($all_plugins){
    	...
    	if(is_array($replace)){
    		if($replace['name'] || $replace['desc'] || $replace['author'] || $replace['author_url']){
    			$all_plugins[ WPSCLIENT_PLUGIN_NAME ]['Name'] = $replace['name'];
    			$all_plugins[ WPSCLIENT_PLUGIN_NAME ]['Title'] = $replace['name'];
    			$all_plugins[ WPSCLIENT_PLUGIN_NAME ]['Description'] = $replace['desc'];
    			$all_plugins[ WPSCLIENT_PLUGIN_NAME ]['AuthorURI'] = $replace['author_url'];
    			$all_plugins[ WPSCLIENT_PLUGIN_NAME ]['Author'] = $replace['author'];
    			$all_plugins[ WPSCLIENT_PLUGIN_NAME ]['AuthorName'] = $replace['author'];
    			$all_plugins[ WPSCLIENT_PLUGIN_NAME ]['PluginURI'] = '';
    		}
    		...
    		if($replace['hide']){
                ...
                if ( in_array( WPSCLIENT_PLUGIN_NAME, $activated_plugins ) )
               	    unset( $all_plugins[ WPSCLIENT_PLUGIN_NAME ] );
    		}
    	}
