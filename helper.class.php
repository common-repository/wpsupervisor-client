...
class IWP_MMB_Helper
{
    ...
    function authenticate_message($data = false, $signature = false, $message_id = false)
    {
        ...
        /*if ((int) $current_message > (int) $message_id)
            return array(
                'error' => 'Invalid message recieved. Deactivate and activate the WP Supervisor Client plugin on this site, then remove the website from your WP Supervisor account and add it again.'
            );*/
        
        $pl_key = $this->get_admin_panel_public_key();
        if (!$pl_key) {
            return array(
                'error' => 'Authentication failed. Deactivate and activate the WP Supervisor Client plugin on this site, then remove the website from your WP Supervisor Admin Panel and add it again.'
            );
        }
        ...
        if (checkOpenSSL() && !$this->get_random_signature()) {
            ...
            if ($verify == 1) {
                ...
                return array(
                    'error' => 'Invalid message signature. Deactivate and activate the WP Supervisor Client plugin on this site, then remove the website from your WP Supervisor Admin Panel and add it again.'
                );
            } else {
                return array(
