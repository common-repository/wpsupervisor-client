<?php
/* 
Plugin Name: WPSupervisor Client
Plugin URI: http://profitmarketer.com/plugins/wps-client
Description: This is the client plugin of WP Supervisor that communicates with the WP Supervisor Admin panel.
Author: Profit Marketer
Version: 1.1.10
Author URI: http://profitmarketer.com
*/

/************************************************************
 * This plugin was modified by Profit Marketer              *
 * Copyright (c) 2013 Profit Marketer                       *
 * www.profitmarketer.com                                   *
 *                                                          *
 ************************************************************/
...
if (version_compare(PHP_VERSION, '5.0.0', '<')) // min version 5 supported
    exit("<p>WP Supervisor Client plugin requires PHP 5 or higher.</p>");

if ( !defined( 'WPSCLIENT_PLUGIN_NAME' ) ) define( 'WPSCLIENT_PLUGIN_NAME', basename( dirname( __FILE__ ) ) . '/wps-client.php' );
...
if( !function_exists ('iwp_mmb_parse_request')) {
	function iwp_mmb_parse_request()
	{
		...
		if(!empty($HTTP_RAW_POST_DATA)) {
			...
			if(strpos($HTTP_RAW_POST_DATA, '-') !== false || strpos($HTTP_RAW_POST_DATA, '_') !== false) {
				$data = @base64_decode(strtr($HTTP_RAW_POST_DATA, '-_', '+/'));
			}
			else {
			...
			if (!get_option('iwp_client_action_message_id') && !get_option('iwp_client_public_key')) {
				...
				if (checkOpenSSL() && !$user_random_key_signing) {
					...
					if (!get_option('iwp_client_nossl_key')) {
						...
						iwp_mmb_response('Please deactivate & activate WP Supervisor Client plugin on your site, then add the site again.', false);
				}
			} else {
				iwp_mmb_response('Please deactivate &amp; activate WP Supervisor Client plugin on your site, then add the site again.', false);
			}
	...
	function iwp_mmb_remove_site($params)
	{
		...
		if (!is_plugin_active($plugin_slug))
			iwp_mmb_response(array(
				'deactivated' => 'Site removed successfully. <br /><br />WP Supervisor Client plugin successfully deactivated.'
			), true);
		else
			iwp_mmb_response(array(
				'removed_data' => 'Site removed successfully. <br /><br /><b>WP Supervisor Client plugin was not deactivated.</b>'
			), true);
