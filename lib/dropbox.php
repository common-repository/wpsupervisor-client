...
class Dropbox {
	...
	public function upload($file, $path = '',$overwrite=true){
        ...
        if ($memory_available < 0) $memory_available = 0;

        $filesize=filesize($file);
        // ...and if not enough memory or filesize over 50MB then send file in chunks.
        if (    
        	(($file_size + 4*1024*1024) < $memory_available) &&
    ...
    public function chunked_upload($file, $path = '',$overwrite=true){
        ...
        while ($data=fread($file_handle, (1024*1024*30))) {  //1024*1024*30 = 30MB
			iwp_mmb_auto_print('dropbox_chunked_upload');
	...
	public function download( $path, $echo = false, $save_to_filehandle = false ) {
		$url = self::API_CONTENT_URL.self::API_VERSION_URL.'files/'.$this->root.'/'.trim($path,'/');
		if (!$echo)
			// Can save directly to filehandle now...
			return $this->request($url, null, 'GET', null, 0, false, $save_to_filehandle);
		else
	...
	private function request($url, $args = null, $method = 'GET', $filehandle = null, $filesize=0, $echo=false, $save_to_filehandle = false){
		...
		if ($echo) {
			...
			// Save directly to filehandle - avoiding out of memory issues...
			if ( $save_to_filehandle ) {
				curl_setopt( $ch, CURLOPT_FILE, $save_to_filehandle );
				$content = curl_exec( $ch ) !== false;
				$output = '';
			}
			else {
				$content = curl_exec($ch);
