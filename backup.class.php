 ...
 * www.prelovac.com
 **************************************************************/
define('IWP_BACKUP_DIR', WP_CONTENT_DIR . '/wpsupervisor/backups');
...
class IWP_MMB_Backup extends IWP_MMB_Core
{
    ...
    function set_memory()
      ...
      if ( (int) @ini_get('max_execution_time') < 3600 ) {
     	  	@ini_set('max_execution_time', 3600); // an hour //twenty minutes
			@set_time_limit(3600); 
...
function delete_task_now($task_name){
    ...
    function backup($args, $task_name = false)
    {
        ...
        $backup_url  = WP_CONTENT_URL . '/wpsupervisor/backups/' . $this->site_name . '_' . $label . '_' . $what . '_' . date('Y-m-d') . '_' . $hash . '.zip';
...
if (isset($account_info['iwp_ftp']) && !empty($account_info['iwp_ftp'])) {
    ...
    function backup_full($task_name, $backup_file, $exclude = array(), $include = array())
    {
        ...
        $remove = array(
            trim(basename(WP_CONTENT_DIR)) . "/wpsupervisor/backups",
...
function ftp_backup($args)
    ...
    function dropbox_backup($args){
        ...
        if(isset($consumer_secret) && !empty($consumer_secret)){
          ...
          return true;
			...
			return array(
                'error' => 'Please connect your WPSupervisor panel with your Dropbox account.'
            );
        }
  ...
  function get_dropbox_backup($args) {
        ...
        if ($dropbox_site_folder == true)
  		...
  		try {
            // Causes memory issues with large donwloads...
            /*
			...
			fclose($handle);
            */
            // Try direct-to-file download instead...
            $handle = @fopen( $temp, 'w' );
            $result = $dropbox->download( $dropbox_destination . '/' . $backup_file, false, $handle );
            fclose($handle);
